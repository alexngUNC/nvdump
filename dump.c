/**
  /rintf("g_sm_control: %p\n", g_sm_control);
  printf("g_sm_control: %p\n", g_sm_control);
...
#endif // __x86_64__
  // Set my_ptr to whatever you want
  //uint64_t* my_ptr = (uint64_t*) ((struct global_sm_control*) (globals_addr + 4728));
  // uint64_t* my_ptr = (uint64_t*) &((struct global_sm_control*) (globals_addr + 4888))->mask;
  uint8_t* my_ptr;
  uint64_t* sm_mask_lower;
  for (int i=-1000; i<1; i++) {
    sm_mask_lower = &( ((struct global_sm_control*)  (globals_addr + 4888))->mask);
    my_ptr = ((uint8_t*) sm_mask_lower) + i;
    fprintf(stdout, "i: %d\nmy_ptr: %p\n", i, my_ptr);
    fprintf(stdout, "*my_ptr: %u\n\n", *my_ptr);
  }
  g_my_ptr = sm_mask_lower;

  // print out bits
  // fprintf(stdout, "my_ptr: %p\n", my_ptr);
  // fprintf(stdout, "*my_ptr: %lu\n", *my_ptr);
...
static void launchCallback(void *ukwn, int domain, int cbid, const void *in_params) {
	...
	fprintf(stdout, "lower mask: %x\n", *lower_ptr);
	fprintf(stdout, "upper mask: %x\n", *upper_ptr);
}
...
void libsmctrl_set_global_mask(uint64_t mask) {
  ...
  printf("g_sm_control: %p\n", g_sm_control);
  //printf("*g_sm_control: %lu\n", *((uint64_t*)g_sm_control));
  printf("g_sm_control->mask: %lu\n", g_sm_control->mask);
  printf("g_sm_control->enabled: %u\n", g_sm_control->enabled);
  fprintf(stdout, "g_my_ptr: %p\n", g_my_ptr);
  fprintf(stdout, "*g_my_ptr: %lu\n", *g_my_ptr);
}
